...
fun ErrorScreen(message: String, onRetry: (() -> Unit)? = null, modifier: Modifier = Modifier) {
    Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
        Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.padding(32.dp)) {
            Icon(Icons.Outlined.ErrorOutline, "Error", modifier = Modifier.size(64.dp), tint = MaterialTheme.colorScheme.error)
            ...
            if (onRetry != null) {
                ...
                TdcButton(text = "Retry", onClick = onRetry, modifier = Modifier.width(160.dp))
            }
        }
    }
}
